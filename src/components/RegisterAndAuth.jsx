...
const RegisterAndAuth = () => {
  ...
  const handleRegister = async () => {
    try {
      ...
      setMessage('Registered successfully. Wait for access token...');
      await getToken(clientID, clientSecret);
    } catch (err) {
      setError('Registration failed. Credentials already registered');
    }
  };
  ...
  const getToken = async (clientID, clientSecret) => {
    try {
      ...
      setMessage('Access token retrieved and stored.');
    } catch (err) {
  ...
  return (
    <Box sx={{ maxWidth: 600, mx: 'auto', mt: 5 }}>
      <Paper sx={{ p: 4 }}>
        <Typography variant="h5" gutterBottom>Registration & Authentication </Typography>
